...
async def fetch_first_page_and_return_html_with_pages_count(browser: Browser):
    ...
    try:
        await page.goto(urljoin(settings.BASE_URL, f"?page=0&countpage={settings.RESULTS_PER_PAGE}"))
        ...
        return html, results_count // settings.RESULTS_PER_PAGE + (results_count % settings.RESULTS_PER_PAGE > 0)
...
async def fetch_page(session, page_number=1):
    ...
    params = {"page": page_number, "countpage": settings.RESULTS_PER_PAGE}
    async with session.get(settings.BASE_URL, headers=headers, params=params) as response:
...
async def parse_car_details(session, car_url):
    ...
    try:
        ...
        async with session.get(car_url) as response:
            if response.status != 200:
                logger.error(f"Failed to fetch car details from {car_url}: {response.status}")
                return
            ...
            raw_phone_number = await fetch_with_retry(
                session,
                settings.PHONE_NUMBER_API_URL.format(car_id=car_id, hash=data_hash, expires=data_expires),
                retries=5,
                delay=10,
                response_type="json",
            )
...
async def limited_parse_car_details(session, car_url, semaphore):
    """
    Parse car details with a semaphore to limit concurrent requests.
    """
    async with semaphore:
        return await parse_car_details(session, car_url)
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                parse_and_save_to_db_tasks = [
                    limited_parse_car_details(session, car_url, semaphore) for car_url in cars_urls
                ]
                ...
                os.makedirs(settings.DATABASE_DUMPS_PATH, exist_ok=True)
                dump_file_path = os.path.join(settings.DATABASE_DUMPS_PATH, f"dump_{timestamp}.sql")
